...
func (s *SQLiteStore) Verify() ([]Issue, error) {
	...
	checks := []struct {
		...
		{"orphaned_tool_call",
			`SELECT tool_call_id FROM tool_calls t WHERE NOT EXISTS (SELECT 1 FROM sessions s WHERE s.user_id = t.user_id AND s.session_id = t.session_id)`,
			"tool call references a session that no longer exists"},
		{"orphaned_opened_file",
			`SELECT file_id FROM opened_files o WHERE NOT EXISTS (SELECT 1 FROM sessions s WHERE s.user_id = o.user_id AND s.session_id = o.session_id)`,
		...
		{"orphaned_route_trace",
			`SELECT trace_id FROM route_traces r WHERE NOT EXISTS (SELECT 1 FROM sessions s WHERE s.user_id = r.user_id AND s.session_id = r.session_id)`,
			"route trace references a session that no longer exists"},
		{"orphaned_workflow_run",
			`SELECT workflow_id FROM workflow_runs w WHERE NOT EXISTS (SELECT 1 FROM sessions s WHERE s.user_id = w.user_id AND s.session_id = w.session_id)`,
			"workflow references a session that no longer exists"},
		{"orphaned_summarization_log",
			`SELECT log_id FROM summarization_logs l WHERE NOT EXISTS (SELECT 1 FROM sessions s WHERE s.user_id = l.user_id AND s.session_id = l.session_id)`,
			"summarization log references a session that no longer exists"},
		{"orphaned_task_schedule",
			`SELECT schedule_id FROM task_schedules t WHERE NOT EXISTS (SELECT 1 FROM sessions s WHERE s.user_id = t.user_id AND s.session_id = t.session_id)`,
			"task schedule references a session that no longer exists"},
		{"orphaned_task_schedule_run",
			`SELECT run_id FROM task_schedule_runs r WHERE NOT EXISTS (SELECT 1 FROM task_schedules t WHERE t.user_id = r.user_id AND t.schedule_id = r.schedule_id)`,
